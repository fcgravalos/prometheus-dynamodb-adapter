...
import (
	...
	"github.com/fcgravalos/prometheus-dynamodb-adapter/dynamodb"
...
func parseFlags() *config {
	...
	//dynamodb.ParseFlags(&cfg.dynamodbConfig)
	...
	flag.StringVar(&cfg.dynamodbTable, "dynamodb.table", "metrics", "DynamoDb Table where metrics will be stored")
	flag.Parse()
...
func buildClients(logger log.Logger, cfg *config) ([]writer, []reader) {
	...
	dynamodbClient := dynamodb.NewClient(logger, cfg.dynamodbTable)
	writers = append(writers, dynamodbClient)
	readers = append(readers, dynamodbClient)
